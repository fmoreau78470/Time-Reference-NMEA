...
Ce projet nécessite un matériel spécifique pour capturer le signal GPS et le signal de synchronisation (PPS).

## Liste des composants (BOM)

1.  **Microcontrôleur :** Raspberry Pi Pico (RP2040) ou RP2040-Zero (plus compact).
2.  **Module GPS :** u-blox NEO-6M, NEO-8M ou équivalent avec sortie PPS.
3.  **Câblage :** 4 fils (VCC, GND, TX, PPS).

## Schéma de câblage

| GPS Pin | RP2040 Pin | Fonction |
| :--- | :--- | :--- |
